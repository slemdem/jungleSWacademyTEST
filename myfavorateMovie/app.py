...
def show_movies_trashed():
    ...
    # 1. db에서 trashed 가 True인 movies 목록을 검색합니다. 주어진 정렬 방식으로 정렬합니다.
    # 참고) find({},{}), sort()를 활용하면 됨.
    #      개봉일 순서 정렬처럼 여러 기준으로 순서대로 정렬해야되는 경우 sort([('A', 1), ('B', 1)]) 처럼 줄 수 있음.
    if(sortMode=='likes') :
...
def trash_movie():
    ...
    # 1. movies 목록에서 find_one으로 영화 하나를 찾아 trashed를 true로 변경합니다.
    result = db.movies.update_one({'_id': id_receive}, {'$set': {'trashed': True}})
    
    # 2. result.updated_count 가 1이면(수정을 한번 실행하였을 때)  result = success 를 보냄
    if result.modified_count == 1:
        return jsonify({'result': 'success'})
    else:
        return jsonify({'result': 'failure'})
...
def restore_movie():
    ...
    # 1. movies 목록에서 find_one으로 영화 하나를 찾아 trashed를 false로 변경합니다.
    result = db.movies.update_one({'_id': id_receive}, {'$set': {'trashed': False}})
...
def delete_movie():
    ...
    id_receive = ObjectId(request.form['id_give'])
    # mongoDB에서 데이터 삭제
    result = db.movies.delete_one({'_id': id_receive})
    
    
    # 2. result.deleted_count 가 1이면(삭제를 한번 실행하였을 때)  result = success 를 보냄
